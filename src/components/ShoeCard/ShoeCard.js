  ...
  return (
    <Link href={`/shoe/${slug}`}>
      <Wrapper>
        ...
        { variant !== 'default' && <Banner variant={variant}>{variantText}</Banner>}
        <Spacer size={12} />
        <Row>
          <Name>{name}</Name>
...
const Price = styled.span`
  margin-left: auto;
  color: ${p => p.variant === 'on-sale' ? COLORS.gray700 : COLORS.gray900};
  text-decoration: ${p => p.variant === 'on-sale' ? 'line-through' : 'inherit'};
`;
